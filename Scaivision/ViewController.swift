...
class ViewController: UIViewController {
    ...
    var galleryButtonImage = UIImage()
...
extension ViewController{
    ...
    override func viewDidLoad() {
        ...
        func configureButtonStyle(){
            ...
            galleryButtonImage = (UIImage(systemName: "photo.fill")?.withTintColor(.white, renderingMode: .alwaysOriginal))!
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if cameraController.currentCameraPosition == .noCamera{
            return
        } else {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let pickedImage = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
          // let scaledImage = self.resizeImage(image: pickedImage)
            ...
            self.view.addSubview(self.pickedImageView)
            SVProgressHUD.show()
            AWSS3Manager.shared.uploadImage(image: self.pickedImageView.image!) {(uploadedFileUrl, error) in
                if let finalPath = uploadedFileUrl as? String {
                    let s3Filepath = finalPath
                    let storyBoard : UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
                    let infoViewController = storyBoard.instantiateViewController(withIdentifier: "infoView") as! InfoViewController
